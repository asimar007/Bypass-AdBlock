...
const App: React.FC = () => {
  ...
  const toggleSite = () => {
    const next = isEnabled ? sites.filter((s) => s !== currentHost) : [...sites, currentHost];
    setSites(next);
  ...
  const runBypass = () => {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs: chrome.tabs.Tab[]) => {
      ...
      if (!tabId) {
        setStatus({ text: "No active tab", ok: false });
        return;
      }
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "12px 16px", borderBottom: "1px solid #ffffff08" }}>
            ...
            >
              ...
              <span
                style={{
                  position: "absolute",
                  inset: 0,
                  background: isEnabled ? "#4caf50" : "#2a2a3a",
                  borderRadius: 22,
        ...
        {isEnabled && (
          <div style={{ marginTop: 6, fontSize: 10, color: "#4caf50" }}>
            ● Active — auto-bypass on every visit
          </div>
        )}
      </div>
      ...
      <div
        style={{
          padding: "12px 16px",
          borderBottom: sites.length > 0 ? "1px solid #ffffff08" : "none",
        }}
      >
        {isEnabled ? (
          <button
      ...
      {sites.length > 0 && (
        <div style={{ padding: "10px 16px 14px" }}>
          ...
          {sites.map((site) => (
            ...
            >
              <span style={{ fontSize: 11, color: site === currentHost ? "#81c784" : "#aaa" }}>
                {site}
              </span>
              <button
